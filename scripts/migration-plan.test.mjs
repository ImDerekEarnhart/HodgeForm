...
test("production tree contains auth and gate migrations",()=>{const files=readdirSync(new URL("../migrations",import.meta.url)).filter(x=>x.endsWith(".sql"));assert.deepEqual(files.sort(),["0001_auth.sql","0002_gate.sql","0003_api_tokens.sql","0004_workspaces.sql","0005_verifier_registry.sql"]);});
