...
try{
 ...
 const reset=await nextEmail("Reset your HodgeForm password",verify.count);const resetLink=await fetch(firstUrl(reset.mail.text),{redirect:"manual"});assert.ok([302,303,307].includes(resetLink.status),`reset email link failed ${resetLink.status}`);const resetLocation=resetLink.headers.get("location");assert.ok(resetLocation,"reset email link must redirect to the requested callback");const resetUrl=new URL(resetLocation,base);const token=resetUrl.searchParams.get("token");assert.ok(token,"password reset callback must carry token");
