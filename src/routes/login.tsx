...
function Login() {
  ...
  return <div className="grid min-h-dvh place-items-center p-5"><div className="w-full max-w-md">
    ...
    <form onSubmit={submit} className="rounded-2xl border border-border bg-bg-elevated p-6 shadow-2xl shadow-black/20">
      ...
      <div className="mt-6 space-y-3">{mode === "signup" && <Input value={name} onChange={(e)=>setName(e.target.value)} placeholder="Name" autoComplete="name" />}<Input type="email" required value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="you@company.com" autoComplete="email" />{mode !== "forgot" && <Input type="password" required minLength={12} value={password} onChange={(e)=>setPassword(e.target.value)} placeholder="Password (12+ characters)" autoComplete={mode === "signin" ? "current-password" : "new-password"} />}</div>
      {error && <p className="mt-3 text-sm text-red-300">{error}</p>}{notice&&<p className="mt-3 text-sm text-emerald-300">{notice}</p>}
      <Button className="mt-5 w-full" disabled={busy}>{busy ? "Working…" : mode === "forgot" ? "Send reset link" : title}</Button>
      <div className="mt-4 flex justify-between text-xs text-muted"><button type="button" className="hover:text-fg" onClick={()=>setMode(mode === "signup" ? "signin" : "signup")}>{mode === "signup" ? "Already have an account?" : "Create account"}</button><button type="button" className="hover:text-fg" onClick={()=>setMode(mode === "forgot" ? "signin" : "forgot")}>{mode === "forgot" ? "Back to sign in" : "Forgot password?"}</button></div>
    </form>
  </div></div>;
}
